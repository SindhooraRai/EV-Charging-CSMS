...
import { Bell, Zap, IndianRupee, AlertTriangle, Cpu } from "lucide-react";
import { PageHeader } from "@/components/shared/page-header";
import { notifications } from "@/lib/mock";

export const Route = createFileRoute("/app/notifications")({
  head: () => ({ meta: [{ title: "Notifications · VoltGrid" }] }),
  component: Notifs,
});
...
function Notifs() {
  return (
    <div>
      <PageHeader title="Notifications" subtitle="Alerts, receipts, and system updates in one feed." />
            ...
            <motion.div key={n.id} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {!n.read && <span className="h-1.5 w-1.5 rounded-full bg-primary-glow" />}
                </div>
